...
def plot_waveform(waveform, sample_rate):
    ...
    print('waveform plot saved in waveform.png')
...
def load_audio_text_image_video(data_or_path_or_list, fs: int = 16000, audio_fs: int = 16000, data_type="sound", tokenizer=None, **kwargs):
    ...
    # return load_audio_text_image_video(data_or_path_or_list, fs=fs, audio_fs=audio_fs, data_type=data_type_j, tokenizer=tokenizer, **kwargs)
    ...
    if isinstance(data_or_path_or_list, str) and os.path.exists(data_or_path_or_list): # local file
        if data_type is None or data_type == "sound":
            ...
            try:
                ...
                data_or_path_or_list, audio_fs = torchaudio.load(data_or_path_or_list, buffer_size=8192, frame_offset=3)
                # plot_waveform(data_or_path_or_list, audio_fs)
                
                # print('read data cost time: ', time.time()-start_time__)
                # TODO 这里添加数据处理代码
                if kwargs.get("reduce_channels", True):
                    data_or_path_or_list = data_or_path_or_list.mean(0)
            except:
    ...
    if audio_fs != fs and data_type != "text":
        ...
        # 划分片段
        # 将data_or_path_or_list划分为多个片段，每个片段的长度为3*fs
        # 片段之间没有重叠部分
        split_data = []
        split_len = 180*fs
        n = data_or_path_or_list.shape[0] // split_len
        for i in range(n):
            split_data.append(data_or_path_or_list[i*split_len:(i+1)*split_len])
        
        
        # 从split_data中随机选择50%片段
        sample_n = int(len(split_data)*0.45)
        indices = np.random.choice(len(split_data), sample_n, replace=False)
