...
def main():
    ...
    train_dataset = MimicFullDataset(
        data_args.version, "train", data_args.max_seq_length, tokenizer, 30, 4)  # TODO delete 30 and 8
    dev_dataset = MimicFullDataset(
        data_args.version, "dev", data_args.max_seq_length, tokenizer, 30, 4)
    eval_dataset = MimicFullDataset(
        data_args.version, "test", data_args.max_seq_length, tokenizer, 30, 4)
    ...
    if model_args.finetune_terms != 'no':
        ...
        model = deactivate_relevant_gradients(
            model, trainable_components, verbose=True)
    if config.model_type == "longformer":
        global_attention_strides = data_args.global_attention_strides
        data_collator = DataCollatorForMimic(
            global_attention_mask_size=train_dataset.global_window, global_attention_strides=global_attention_strides)
    ...
    if True:
        ...
        p_train = trainer.predict(train_dataset, metric_key_prefix="predict")
        predict_train = p_train.predictions[0] if isinstance(
            p_train.predictions, tuple) else p_train.predictions
        y_train = p_train.label_ids == config.label_yes

        threshold_train = find_threshold_micro(predict_train, y_train)
    ...
    if training_args.do_predict:
        ...
        for predict_dataset, task in zip(predict_datasets, tasks):
            ...
            if trainer.is_world_process_zero():
                ...
                uniids_toinclude = ["285.9", "276.2", "584.9", "511.9", "38.93", "285.1",
                                    "276.1", "276.2", "287.5", "V15.82", "38.91", "88.72", "37.23"]
                ...
                with open(output_predict_file, "w") as writer:
                    ...
                    for proc, item, testa in zip(y, preds, predict_dataset.df):
                        ...
                        itwm_to_write_row2 = ";".join(
                            set_to_write_row2)  # 真实标签
                        set_to_write_row3 = []
                        for indexx, prob in enumerate(item):  # 模型预测出来的概率
                            # if prob > 0:
                            if prob > threshold:
                                set_to_write_row3 += [label_list[indexx]]
                        itwm_to_write_row3 = ";".join(
                            set_to_write_row3)  # 预测标签
                        for indexx, label in enumerate(uniids_toinclude):
                            if label in set_to_write_row2:
                                if label in set_to_write_row3:  # 标签在真实标签里面，并且被预测出来了
                                    to_save_succ[indexx].append(
                                        (label, testa["hadm_id"], testa["LABELS"], itwm_to_write_row2, itwm_to_write_row3, testa["TEXT"]))
                                else:
                                    # 标签在真实标签里面，没有被预测出来
                                    to_save_fail[indexx].append(
                                        (label, testa["hadm_id"], testa["LABELS"], itwm_to_write_row2, itwm_to_write_row3, testa["TEXT"]))
                            judge = label in set_to_write_row3 and (
                                not (label in set_to_write_row2))
                            if judge:  # 被预测出来的标签没有在真实标签里面
                                print(judge)
                                to_save_eror[indexx].append(
                                    (label, testa["hadm_id"], testa["LABELS"], itwm_to_write_row2, itwm_to_write_row3, testa["TEXT"]))
                        tmp = testa["LABELS"]
                        writer.write(
                            f"{index}\t{tmp}\t{itwm_to_write_row2}\t{itwm_to_write_row3}\n")
                        writer_list_to_json.append(
                            {"index": index, "true1": tmp, "true2": itwm_to_write_row2, "prediction": itwm_to_write_row3})
                        output_predict_file_json = os.path.join(
                            training_args.output_dir, f"predict_results_{task}.json")
                        json.dump(writer_list_to_json, open(
                            output_predict_file_json, "w"))
                        index += 1
                for a, b, c, d in zip(uniids_toinclude, to_save_succ, to_save_fail, to_save_eror):
                    with open("./case_study/predict_results_mimic3_succ_%s.txt" % (a), "w") as writer:
                        for bb in b:
                            writer.write(
                                f"x-x-x-\n{bb[1]}\t{bb[2]}\t{bb[3]}\t{bb[4]}\n{bb[5]}\n")
                    with open("./case_study/predict_results_mimic3_fail_%s.txt" % (a), "w") as writer:
                        for cc in c:
                            writer.write(
                                f"x-x-x-\n{cc[1]}\t{cc[2]}\t{cc[3]}\t{cc[4]}\n{cc[5]}\n")
                    with open("./case_study/predict_results_mimic3_eror_%s.txt" % (a), "w") as writer:
                        for dd in d:
                            writer.write(
                                f"x-x-x-\n{dd[1]}\t{dd[2]}\t{dd[3]}\t{dd[4]}\n{dd[5]}\n")
